...
async def update_guest_seat(guest_name: str, new_seat: str) -> str:
    ...
    search_url = "http://localhost:8000/api/guest/search"
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            # Search for guest
            async with session.post(search_url, json=payload, timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status != 200:
                ...
                full_name = guest_info.get("name", guest_name)
                old_seat = guest_info.get("table_number", "none")
                ...
                async with session.put(
                    ...
                    if update_response.status == 200 and update_data.get("success"):
                        return f"Successfully updated {full_name}'s seat from {old_seat} to {new_seat}"
                    else:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(search_url, json=payload, timeout=aiohttp.ClientTimeout(total=5)) as response:
                ...
                guest_id = guest_info.get("id")
                full_name = guest_info.get("name", guest_name)
                old_phone = guest_info.get("phone", "none")
                ...
                async with session.put(
                    ...
                    if update_response.status == 200 and update_data.get("success"):
                        return f"Successfully updated {full_name}'s phone from {old_phone} to {new_phone}"
                    else:
                        error_msg = update_data.get("message", "Unknown error")
                        return f"Failed to update phone: {error_msg}"
